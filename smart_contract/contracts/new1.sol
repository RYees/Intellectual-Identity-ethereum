...
contract IP {
    ...
    function mintnft(uint index, string memory tokenURI) external returns(uint256) {
        ...
        return nft.createToken(tokenURI);
    }
    ...
    function BidPay(string memory _ownerIPname, uint256 _tokenId) public returns (bool) {
        return nft.BidPay(_ownerIPname, _tokenId);
    }

    function payMeBack(uint amountToWithdraw) public returns (bool) {
        return nft.payMeBack(amountToWithdraw);
    }

    function executeSale(uint256 tokenId) public {
        return nft.executeSale(tokenId);
    }
